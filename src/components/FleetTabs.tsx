...
export function FleetTabs({ locale }: { locale: string }) {
  ...
  return (
    <div>
      <div className="mb-8 inline-flex rounded-xl border border-line bg-paper-2 p-1">
        <button
          type="button"
          className={`min-h-11 rounded-lg px-5 text-sm font-bold ${tab === "bikes" ? "bg-navy text-white" : "text-ink-soft"}`}
        ...
        <button
          type="button"
          className={`min-h-11 rounded-lg px-5 text-sm font-bold ${tab === "cars" ? "bg-navy text-white" : "text-ink-soft"}`}
          onClick={() => setTab("cars")}
        >
          {ru ? "4×4" : "Overland 4×4"}
        </button>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {bikes.map((bike) => (
            <article key={bike.slug} className="feature-card">
              <img src={bike.images[0]} alt={`${bike.model} ${bike.unitNumber}`} className="h-48 w-full object-cover" />
              <div className="p-5">
